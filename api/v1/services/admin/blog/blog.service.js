...
const BlogTag = require('../../../models/blog/blogTag.model')
...
function normalizeTags(value) {
  ...
  return Array.from(new Set(tags.map(tag => normalizeText(tag)).filter(Boolean))).slice(0, 24)
}
...
async function buildUniqueSlug({ title, slugInput, currentId = null }) {
  ...
  const baseSlug =
    slugify(normalizeText(slugSource), {
      lower: true,
      strict: true,
      locale: 'vi'
    }) || `blog-${Date.now()}`
...
async function resolveTags({ tagIds, tags }) {
  ...
  if (ids.length > 0)
    return BlogTag.find({ _id: { $in: ids }, deleted: false, status: 'active' })
      .select('_id name slug translations')
      .lean()
...
async function resolveCategory({ categorySlug, categoryRef, categoryName }) {
  ...
  if (name) {
    return BlogCategory.findOne({
      isActive: true,
      $or: [{ name }, { slug: name }, { 'translations.en.name': name }]
    })
...
async function buildListQuery(params = {}) {
  ...
  if (category) {
    ...
    appendAndFilter(query, {
      $or: [{ category: categorySearch }, { 'translations.en.category': categorySearch }]
    })
  }
  ...
  if (tag) {
    ...
    appendAndFilter(query, {
      $or: [...(tagDoc?._id ? [{ tagIds: tagDoc._id }] : []), { tags: tag }, { 'translations.en.tags': tag }]
    })
  }
...
async function listBlogPosts(params = {}) {
  ...
  const posts = await blogPostRepository.findByQuery(query, {
    ...
    populate: [
      ...
      { path: 'updatedBy', select: 'fullName username email' },
      { path: 'publishedBy', select: 'fullName username email' }
    ]
  })
...
async function createBlogPost(payload = {}, user = null) {
  ...
  const reviewStatus = normalizeReviewStatus(payload.reviewStatus, status === 'published' ? 'approved' : 'pending')
...
async function updateBlogPost(id, payload = {}, user = null) {
  ...
  const nextStatus = hasOwn(payload, 'status') ? normalizeStatus(payload.status, currentPost.status) : currentPost.status
  ...
  if (
    hasOwn(payload, 'seo') ||
    hasOwn(payload, 'seoTitle') ||
    hasOwn(payload, 'seoDescription') ||
    hasOwn(payload, 'seoKeywords') ||
    hasOwn(payload, 'canonicalUrl')
  ) {
  ...
  try {
    await createRevision({
      ...
      snapshot: currentPost.toObject ? currentPost.toObject() : currentPost,
