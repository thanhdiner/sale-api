...
async function getHomeWhyChooseUsContent(language = 'vi') {
  const normalizedLanguage = normalizeLanguage(language)

  return cache.getOrSet(
    `home-why-choose-us:content:${normalizedLanguage}`,
    async () => {
      const content = await homeWhyChooseUsContentRepository.findOne({ lean: true })
    ...
    TTL_HOME_WHY_CHOOSE_US_CONTENT
  )
}
